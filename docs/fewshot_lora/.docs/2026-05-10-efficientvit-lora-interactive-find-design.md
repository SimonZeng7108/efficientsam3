...
结构整理后又做了两处小收口，外部 CLI 行为不变：

- 新增 `fewshot_lora/types.py`，把 `TrainRoundOutput` 从 `runtime.loop` 移到根级中立 DTO 模块。
  `runtime.loop` 和 `sam3_integration.training` 都从这里导入，避免 SAM3 集成层反向依赖 runtime 调度层。
- `sam3_integration/factory.py` 的模型构建函数改为公开名 `build_trainable_model()`。
  为兼容旧内部调用，文件末尾保留 `_build_trainable_model = build_trainable_model` 别名，但新代码和测试使用公开名。

这次收口后需要额外检查：

```text
pytest tests/fewshot_lora -q
python -m compileall -q fewshot_lora tests\fewshot_lora
```
