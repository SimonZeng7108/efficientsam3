...
当前 MVP 默认走“完整 EfficientSAM3 -> task visual prompt / adapter -> SAM3 原生 decoder 输出”的路径。旧的 `proposal_candidates.json` / 外置 head 实现已经从代码中清理，不再是产品验证主线。每轮训练后直接对全量图片执行原生前向，经过单图同类 NMS 后写入 `predictions.json`，再由真值匹配模块筛选漏检、误检、定位错误和低置信正确样本。
...
- 过滤面积太大的异常目标。
- 根据目标先验过滤长宽比。
- 用 rotated IoU 做 NMS，默认阈值由 `EVAL.NMS_IOU_THRESHOLD` 控制；如果预测没有可用 OBB，则回退 HBB IoU。
...
- 如果最终输出是 OBB，优先使用 rotated IoU；DataTrain 的四点 polygon 会先拟合 OBB，不会直接退回 HBB。
- 如果有 polygon 或 mask，额外计算 mask IoU 或 polygon IoU。
- 当多个预测匹配同一真值时，保留 IoU 最高者，其余视为重复误检。
- 默认匹配阈值可从 `OBB IoU >= 0.5` 开始，后续按业务调节。
- `EVAL.LOW_CONFIDENCE_THRESHOLD` 控制低置信正确样本，默认 0.4；这类样本已经匹配成功，但会作为低优先级不稳定样本进入 `errors.json`。
...
- 最后才选择低置信正确样本，用于补强已经基本检测对但分数不稳的目标。
