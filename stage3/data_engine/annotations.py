...
def build_qwen_labeling_messages(
    ...
    system_prompt = (
        "You are labeling one segmentation mask for a Stage 3 image text-mask pair data engine. "
        "Return JSON only with keys: label, confidence, ambiguous, reject_reason. "
        "Describe the main visible object or object part in the cropped image as specifically as possible. "
        ...
        "If the crop is too ambiguous, too small, or unreadable, set ambiguous=true and provide a short reject_reason. "
...
def _annotation_structure_text(index: int, annotation: Dict[str, Any]) -> str:
    ...
    lines = [f"{entry_kind}[{entry_id}]", f"text: {label}"]

    if annotation.get("object_ids_output") is not None:
        lines.append(
            f"object_ids_output: {_format_numeric_list(annotation['object_ids_output'])}"
        )
    if annotation.get("mask_id") is not None:
        lines.append(f"mask_id: {annotation['mask_id']}")
    if annotation.get("source_mask_ids") is not None:
        lines.append(f"source_mask_ids: {annotation['source_mask_ids']}")
    if annotation.get("area") is not None:
        lines.append(f"area_px: {_format_numeric(annotation['area'])}")
    if annotation.get("bbox_xywh") is not None:
        lines.append(f"bbox_xywh: {_format_numeric_list(annotation['bbox_xywh'])}")
    crop_box = _resolve_crop_box_xyxy(annotation, image_size=(10**9, 10**9))
    if crop_box is not None:
        lines.append(f"crop_xyxy: {_format_numeric_list(crop_box)}")
    return "\n".join(lines)
