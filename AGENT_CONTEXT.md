...
- 当前闭环还没有 no-object 负样本训练结构；纯背景误检会进入 `errors.json`、`errors_vis/` 和 `predictions_vis/`，但不会作为背景训练样本更新 adapter。
...
每轮输出目录里会额外生成 `train_inputs/`、`errors_vis/`、`predictions_vis/`。`train_inputs/` 目前只代表进入训练的正样本；纯背景误检图要在 `errors_vis/` 或 `predictions_vis/` 查看。
